...
// ================================================================================================================================================
describe('GET /energy/api/ActualvsForecast/../month/YYYY-MM', () => {
    ...
    // ---------------------------------------------------------------------------------------------------------------------------
    it('Should create 200 - OK (ADMIN)', async done => {
        request.get('/energy/api/ActualvsForecast/Greece/PT60M/month/2018-01?format=csv')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(200)
        .end(async (err, res) => {
            if(!err){
             ...
             try{
                ...
                expect(fields[10]).toBe('ActualTotalLoadByDayValue');
                ...
                for(var i=1; i<11; i++){
                    ...
                    expect(row[10]).not.toBe(null);                 
                    expect(row[10]).toBe(r1[0].total.toString());                                               // and TotalLoadValues must agree
                    let r2 = await DayAheadTotalLoadForecast.aggregate([
                        { $match: {"AreaName": 'Greece', "Year": 2018, "Month": 1, "Day": i, "ResolutionCodeId": 2} },
                    ...
                    expect(row[9]).not.toBe(null);                                                                                              // should not be null
                    expect(row[9]).toBe(r2[0].total.toString());   
    ...
    it('Should create 400 - BAD REQUEST(RESOLUTION)', async done => {
        request.get('/energy/api/ActualvsForecast/Greece/PT40M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (DATE)', async done => {
        request.get('/energy/api/ActualvsForecast/Greece/PT60M/month/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (FORM)', async done => {
        request.get('/energy/api/ActualvsForecast/PT60M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){

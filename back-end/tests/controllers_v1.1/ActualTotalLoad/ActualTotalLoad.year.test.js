...
describe('GET /energy/api/ActualTotalLoad/../year/YYYY-MM-DD', () => {
    ...
    it('Should create 200 - OK (ADMIN)', async done => {
        request.get('/energy/api/ActualTotalLoad/Greece/PT60M/year/2018')
        ...
        .end(async (err, res) => {
            if(!err){
                ...
                expect(res.body.results[0]).toHaveProperty('ActualTotalLoadByMonthValue');
                // ------------------------------------------------------------------------

                // values 
                // ------------------------------------------------------------------------
                expect(res.body.results[0].Source).toBe('entso-e');
                expect(res.body.results[0].Dataset).toBe('ActualTotalLoad');
                ...
                expect(res.body.results[0].ActualTotalLoadByMonthValue).toBe(r[0].total);
                // ------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------------------------------------------------------
    it('Should create 200 - OK (ADMIN)', async done => {
        request.get('/energy/api/ActualTotalLoad/Greece/PT60M/year/2018?format=csv')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(200)
        .end(async (err, res) => {
            if(!err){
             ...
             try{
                ...
                expect(fields[8]).toBe('ActualTotalLoadByMonthValue');
    ...
    it('Should create 400 - BAD REQUEST(RESOLUTION)', async done => {
        request.get('/energy/api/ActualTotalLoad/Greece/PT40M/year/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (DATE)', async done => {
        request.get('/energy/api/ActualTotalLoad/Greece/PT60M/year/201')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (FORM)', async done => {
        request.get('/energy/api/ActualTotalLoad/PT60M/year/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - AREANAME', async done => {
        request.get('/energy/api/ActualTotalLoad/Aust/PT60M/year/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - TIMERES', async done => {
        request.get('/energy/api/ActualTotalLoad/Greece/PT30M/year/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - YEAR', async done => {
        request.get('/energy/api/ActualTotalLoad/Greece/PT60M/year/2028')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){

...
describe('GET /energy/api/ActualTotalLoad/../year/YYYY-MM-DD', () => {
    ...
    it('Should create 200 - OK (ADMIN)', async done => {
        request.get('/energy/api/ActualTotalLoad/Greece/PT60M/month/2018-01')
        ...
        .end(async (err, res) => {
            if(!err){
                ...
                for(i=0; i<10; i++){
                    ...
                    expect(res.body.results[i].ActualTotalLoadByDayValue).toBe(r[0].total);
                    // ------------------------------------------------------------------------
    ...
    it('Should create 400 - BAD REQUEST(RESOLUTION)', async done => {
        request.get('/energy/api/ActualTotalLoad/Greece/PT40M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (DATE)', async done => {
        request.get('/energy/api/ActualTotalLoad/Greece/PT60M/month/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (FORM)', async done => {
        request.get('/energy/api/ActualTotalLoad/PT60M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - AREANAME', async done => {
        request.get('/energy/api/ActualTotalLoad/Aust/PT60M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - TIMERES', async done => {
        request.get('/energy/api/ActualTotalLoad/Greece/PT30M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - MONTH', async done => {
        request.get('/energy/api/ActualTotalLoad/Greece/PT60M/month/2018-05')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){

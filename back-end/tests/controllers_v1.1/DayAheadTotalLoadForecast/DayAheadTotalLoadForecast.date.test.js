...
// ================================================================================================================================================
describe('GET /energy/api/DayAheadTotalLoadForecast/../year/YYYY-MM-DD', () => {
    ...
    it('Should create 200 - OK (ADMIN)', async done => {
        request.get('/energy/api/DayAheadTotalLoadForecast/Greece/PT60M/date/2018-01-04')
        ...
        .end(async (err, res) => {
            if(!err){
                ...
                for(i=0; i<24; i++){
                    ...
                    let r = await DayAheadTotalLoadForecast.findOne({AreaName:'Greece', ResolutionCodeId: 2, DateTime: date});    // find the record in database                
                    expect(r).not.toBe(null);                                                                           // should not be null
                    expect(res.body.results[i].DayAheadTotalLoadForecastValue).toBe(r.TotalLoadValue);                            // and TotalLoadValues must agree
    ...
    // ---------------------------------------------------------------------------------------------------------------------------
    it('Should create 200 - OK (ADMIN)', async done => {
        request.get('/energy/api/DayAheadTotalLoadForecast/Greece/PT60M/date/2018-01-04?format=csv')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(200)
        .end(async (err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST(RESOLUTION)', async done => {
        request.get('/energy/api/DayAheadTotalLoadForecast/Greece/PT40M/date/2018-01-04')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (DATE)', async done => {
        request.get('/energy/api/DayAheadTotalLoadForecast/Greece/PT60M/date/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (FORM)', async done => {
        request.get('/energy/api/DayAheadTotalLoadForecast/PT60M/date/2018-01-04')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - AREANAME', async done => {
        request.get('/energy/api/DayAheadTotalLoadForecast/Aust/PT60M/date/2018-01-04')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - TIMERES', async done => {
        request.get('/energy/api/DayAheadTotalLoadForecast/Greece/PT30M/date/2018-01-04')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - DATE', async done => {
        request.get('/energy/api/DayAheadTotalLoadForecast/Greece/PT60M/date/2018-01-14')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){

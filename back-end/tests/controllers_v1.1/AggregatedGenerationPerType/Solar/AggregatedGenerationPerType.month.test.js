...
// ================================================================================================================================================
describe('GET /energy/api/AggregatedGenerationPerType/Greece/Solar/../month/YYYY-MM', () => {
    ...
    it('Should create 200 - OK (ADMIN)', async done => {
        // /energy/api/AggregatedGenerationPerType/Greece/Solar/PT60M/month/2018-01
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT60M/month/2018-01')
        ...
        .end(async (err, res) => {
            if(!err){
                ...
                for(i=0; i<10; i++){
                    ...
                    expect(res.body.results[i]).toHaveProperty('ActualGenerationOutputByDayValue');
                    // ------------------------------------------------------------------------
                    ...
                    let r = await AggregatedGenerationPerType.aggregate([
                        { $match: {"AreaName": 'Greece', "Year": 2018, "Month": 1, "Day": i+1, "ResolutionCodeId": 2, "ProductionTypeId": 17} },
                        
                        { $group: { _id: {Year:"$Year",Month:"$Month",Day:"$Day"}, total: { $sum: "$ActualGenerationOutput" }, count: {$sum: 1} } },
                        
                        { $sort : { _id:1 } }
            
                    ]);
                    expect(res.body.results[i].ActualGenerationOutputByDayValue).toBe(r[0].total);
                    // ------------------------------------------------------------------------
 ...
 // GET - 200 ok by admin
    // ---------------------------------------------------------------------------------------------------------------------------
    it('Should create 200 - OK (ADMIN)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT60M/month/2018-01?format=csv')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(200)
        .end(async (err, res) => {
            if(!err){
             ...
             try{
                ...
                expect(fields[9]).toBe('ProductionType');
                expect(fields[10]).toBe('ActualGenerationOutputByDayValue');
                ...
                for(i=1; i<11; i++){
                    ...
                    expect(row[9]).toBe('Solar');

                    // get the day group value from database
                    let r = await AggregatedGenerationPerType.aggregate([
                        { $match: {"AreaName": 'Greece', "Year": 2018, "Month": 1, "Day": i, "ResolutionCodeId": 2, "ProductionTypeId": 17} },
                    ...
                    expect(row[10]).toBe(r[0].total.toString());
                    // ------------------------------------------------------------------------
    ...
    it('Should create 400 - BAD REQUEST(RESOLUTION)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT40M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (DATE)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT60M/month/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (FORM)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/PT60M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    // ---------------------------------------------------------------------------------------------------------------------------
    it('Should create 403 - AREANAME', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Aust/Solar/PT60M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - TIMERES', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT30M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - PRODUCTIONTYPE', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/hhhhhh/PT60M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - MONTH', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT60M/month/2018-11')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){

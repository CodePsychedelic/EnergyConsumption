...
// ================================================================================================================================================
describe('GET /energy/api/AggregatedGenerationPerType/Greece/Solar/../date/YYYY-MM-DD', () => {
    ...
    it('Should create 200 - OK (ADMIN)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT60M/date/2018-01-04')
    ...
    // ---------------------------------------------------------------------------------------------------------------------------
    it('Should create 200 - OK (ADMIN)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT60M/date/2018-01-04?format=csv')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(200)
        .end(async (err, res) => {
            if(!err){
                ...
                try{
                    ...
                    expect(fields[10]).toBe('ProductionType');
                    expect(fields[11]).toBe('ActualGenerationOutputValue');
                    expect(fields[12]).toBe('UpdateTimeUTC');
    ...
    it('Should create 400 - BAD REQUEST(RESOLUTION)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT40M/date/2018-01-04')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (DATE)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT60M/date/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - AREANAME', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Aust/Solar/PT60M/date/2018-01-04')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    // GET - 403 NO_DATA /energy/api/AggregatedGenerationPerType/Greece/PT30M/date/2018-01-04
    // ---------------------------------------------------------------------------------------------------------------------------
    it('Should create 403 - NO DATA - PRODUCTION TYPE', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/hhhh/PT60M/date/2018-01-04')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA - TIMERES', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT30M/date/2018-01-04')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
    ...
    // GET - 403 NO_DATA /energy/api/AggregatedGenerationPerType/Greece/PT30M/date/2018-01-04
    // ---------------------------------------------------------------------------------------------------------------------------
    it('Should create 403 - NO DATA - DATE', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT60M/date/2018-01-24')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){

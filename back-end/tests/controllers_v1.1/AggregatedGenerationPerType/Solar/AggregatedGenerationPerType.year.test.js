...
// /energy/api/AggregatedGenerationPerType/.. ENDPOINT
// ================================================================================================================================================
describe('GET /energy/api/AggregatedGenerationPerType/Greece/Solar/../year/YYYY', () => {
    ...
    it('Should create 200 - OK (ADMIN)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT60M/year/2018')
        ...
        .end(async (err, res) => {
            if(!err){
                ...
                expect(res.body.results[0]).toHaveProperty('Dataset');
                expect(res.body.results[0]).toHaveProperty('AreaName');
                expect(res.body.results[0]).toHaveProperty('AreaTypeCode');
                expect(res.body.results[0]).toHaveProperty('MapCode');
                expect(res.body.results[0]).toHaveProperty('ResolutionCode');
                expect(res.body.results[0]).toHaveProperty('Year');
                expect(res.body.results[0]).toHaveProperty('Month');
                expect(res.body.results[0]).toHaveProperty('ProductionType');
                expect(res.body.results[0]).toHaveProperty('ActualGenerationOutputByMonthValue');
                // ------------------------------------------------------------------------

                // values 
                // ------------------------------------------------------------------------
                expect(res.body.results[0].Source).toBe('entso-e');
                expect(res.body.results[0].Dataset).toBe('AggregatedGenerationPerType');
                expect(res.body.results[0].AreaName).toBe('Greece');
                expect(res.body.results[0].AreaTypeCode).toBe('CTY');
                expect(res.body.results[0].MapCode).toBe('GR');
                expect(res.body.results[0].ResolutionCode).toBe('PT60M');
                expect(res.body.results[0].Year).toBe(2018);
                expect(res.body.results[0].ProductionType).toBe('Solar');
                let r = await AggregatedGenerationPerType.aggregate([
                    { $match: {"AreaName": 'Greece', "Year": 2018, "ResolutionCodeId": 2, "ProductionTypeId": 17} },
                    
                    { $group: { _id: {Year:"$Year",Month:"$Month"}, total: { $sum: "$ActualGenerationOutput" }, count: {$sum: 1} } },
                    
                    { $sort : { _id:1 } }
                ])
                
                expect(res.body.results[0].ActualGenerationOutputByMonthValue).toBe(r[0].total);
                // ------------------------------------------------------------------------
    ...
    it('Should create 200 - OK (ADMIN)', async done => {
        ...
        .end(async (err, res) => {
            if(!err){
                ...
                const files = fs.readdirSync(process.env.DOWNLOADS + '/AggregatedGenerationPerType');
             ...
             try{
                ...
                expect(fields[8]).toBe('ProductionType');
                expect(fields[9]).toBe('ActualGenerationOutputByMonthValue');
                ...
                expect(row[1]).toBe('AggregatedGenerationPerType');
                ...
                expect(row[8]).toBe('Solar');
              
                let r = await AggregatedGenerationPerType.aggregate([
                    { $match: {"AreaName": 'Greece', "Year": 2018, 'Month': i, "ResolutionCodeId": 2, "ProductionTypeId": 17} },
    ...
    it('Should create 400 - BAD REQUEST(RESOLUTION)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT40M/year/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (DATE)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/Solar/PT60M/year/201')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (FORM)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/PT60M/Solar/year/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){

...
// ================================================================================================================================================
describe('GET /energy/api/AggregatedGenerationPerType/Greece/AllTypes/../year/YYYY', () => {
    ...
    it('Should create 200 - OK (ADMIN)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/AllTypes/PT60M/year/2018')
        ...
        .end(async (err, res) => {
            if(!err){
                ...
                for(var i=0; i<5; i++){
                    ...
                    //console.log(res.body.results[i].ProductionType);
                    ...
                    let r2 = await AggregatedGenerationPerType.aggregate([
                        { $match: {"AreaName": 'Greece', "Year": 2018, "ResolutionCodeId": 2, "ProductionTypeId": r1.Id} },
                    ...
                    expect(res.body.results[i].ActualGenerationOutputByMonthValue).toBe(r2[0].total);
                    // ------------------------------------------------------------------------
                }
    ...
    it('Should create 400 - BAD REQUEST(RESOLUTION)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/AllTypes/PT40M/year/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (DATE)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/AllTypes/PT60M/year/201')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (FORM)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/PT60M/AllTypes/year/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/AllTypes/PT30M/year/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){

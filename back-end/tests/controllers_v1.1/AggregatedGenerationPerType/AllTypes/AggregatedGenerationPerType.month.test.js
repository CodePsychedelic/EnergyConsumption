...
// ================================================================================================================================================
describe('GET /energy/api/AggregatedGenerationPerType/Greece/AllTypes/../month/YYYY-MM', () => {
    ...
    it('Should create 200 - OK (ADMIN)', async done => {
        // /energy/api/AggregatedGenerationPerType/Greece/AllTypes/PT60M/month/2018-01
        request.get('/energy/api/AggregatedGenerationPerType/Greece/AllTypes/PT60M/month/2018-01')
        ...
        .end(async (err, res) => {
            if(!err){
                ...
                for(i=0; i<50; i++){
                    ...
                    expect(res.body.results[i].Day).toBe(d);
                    ...
                    let r2 = await AggregatedGenerationPerType.aggregate([
                        { $match: {"AreaName": 'Greece', "Year": 2018, "Month": 1, "Day": d, "ResolutionCodeId": 2, "ProductionTypeId": r1.Id} },
                    ...
                    expect(res.body.results[i].ActualGenerationOutputByDayValue).toBe(r2[0].total);
                    // ------------------------------------------------------------------------
    ...
    it('Should create 400 - BAD REQUEST(RESOLUTION)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/AllTypes/PT40M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 400 - BAD REQUEST (DATE)', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/AllTypes/PT60M/month/2018')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(400)
        .end((err, res) => {
            if(!err){
    ...
    it('Should create 403 - NO DATA', async done => {
        request.get('/energy/api/AggregatedGenerationPerType/Greece/AllTypes/PT30M/month/2018-01')
        .set({'X_OBSERVATORY_AUTH':token})
        .expect(403)
        .end((err, res) => {
            if(!err){
